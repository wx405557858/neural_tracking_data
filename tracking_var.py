...
from keras.layers import Conv2D, MaxPooling2D, ZeroPadding2D, Convolution2D, UpSampling2D, Add
...
def build_model():
    ...
    model.add(Conv2D(32, (5, 5), activation='relu', padding='same', input_shape=(HEIGHT,WIDTH, 6)))
...
#     input_img = Input(shape=(WIDTH, HEIGHT, 6))
    ...
    conv1 = Conv2D(32, ksize, activation='relu', padding = padding, input_shape=(None, None, 8))(input_img)
...
#         model.fit(X_train, Y_train, validation_data=(X_test, Y_test), batch_size=32, epochs=2)
        model.fit_generator(generate_img(), validation_data = None, steps_per_epoch=2000, epochs=1, workers=4, use_multiprocessing=True)
